...
const SingleCard = ({ data }) => {
  ...
  const { setEditContact, setIsOpenCreateModal, handleSelect, selectedContacts } = useContext(ContactContext);
  ...
  return (
    <div className="flex items-center gap-2 border border-gray-300 bg-white rounded-lg p-[.5rem] h-[6.6rem] shadow relative">
      {/* Checkbox */}
      <div>
        <input 
          type="checkbox" 
      ...
      {/* Profile Initial */}
      <div className={`h-[4rem] w-[4rem] flex items-center justify-center rounded-lg ${cardColor}`}>
      ...
      {/* Edit Icon */}
      <div onClick={handleEdit} className="text-gray-500 cursor-pointer self-end mt-auto absolute bottom-1 right-1">
        <CiEdit size={24} />
      </div>
    </div>
  );
